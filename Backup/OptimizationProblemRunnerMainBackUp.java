            ...
            if (model.get(GRB.IntAttr.Status) == GRB.Status.INFEASIBLE) {
                ...
                try {
                    for (int i = 0; i < numOperations; i++) {
                        ...
                        System.out.println("Operation " + (i + 1) + " (" + operation.getJobType() + " - Step " + operation.getStepNumber() + ")");
                        ...
                        for (int o = 0; o < operation.getProcesses().size(); o++) {
                            ...
                            for (int k : process.getEligibleMachines()) {
                                if (A[i][o][k].get(GRB.DoubleAttr.X) == 1.0) {
                                    String machineName = machineNames.get(k);
                                    System.out.println("  Process : " + processName + " is assigned to " + machineName + " (Machine " + (k + 1) + ")");
                                }
                            }
                        }
                    }
                } catch (GRBException e_1) {
                    e_1.printStackTrace();
                }
